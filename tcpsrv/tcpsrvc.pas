   ...
   while True do begin
       ...
       begin
          ...
          if(f=1) then begin
             ...
             // имя глобальной области памяти для передачи значений
             if(CURCMD ='GLMEMNAME') then begin
                if(Length(CURFLD)>0) then begin
                   GLMEMNAME:=Copy(CURFLD,0,Length(CURFLD));
                end;
             end;
             // имя глобальной области памяти для контроля работы программы
             if(CURCMD ='CPROCGMEM') then begin
                if(Length(CURFLD)>0) then begin
                   PM_GLMEMNAME:=Copy(CURFLD,0,Length(CURFLD));
                end;
             end;
             // номер ячейки в области контроля работы программы
